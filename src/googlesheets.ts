 ...
 */
public async getOhlcArrays(sheetName: string = 'Sheet1'): Promise<{ open: number[], high: number[], low: number[], close: number[], pp: number[] }> {
...
/**
 * Finds the last row in a specific column, adds a timestamp to the next empty cell,
 */
public async updateLastCellWithTimeStampandData(data: string): Promise<void> {
    try {
        // 1. Get all data from the reference column (SHEET_COLUMN) to determine the last row.
        const response = await this.sheets.spreadsheets.values.get({
            spreadsheetId: this.spreadsheetId,
            range: Config.SHEET_COLUMN, // e.g., 'Sheet1!A:A'
        });

        const values = response.data.values;
        // The lastRow is the last filled row index (1-based).
        const lastRow = (values && values.length > 0) ? values.length : 0;
        
        // The new row to write to is lastRow + 1
        const targetRow = lastRow + 1;
        
        const now = new Date();
        const timestamp = now.toLocaleString();

        // 2. Determine the full range to update (e.g., A4:B4)
        // This is necessary to write both the timestamp (in UPDATE_COLUMN) and the data (in the next column)
        
        const startColumnCode = Config.UPDATE_COLUMN.charCodeAt(0);
        
        const nextColumnLetter = String.fromCharCode(startColumnCode + 1);
        
        const updateRange = `${Config.UPDATE_COLUMN}${targetRow}:${nextColumnLetter}${targetRow}`;
        ...
        console.log(`Successfully updated row ${targetRow} with Timestamp: ${timestamp} and Data: ${data}`);
    } catch (err) {
        console.error('ERROR UPDATING', err);
    }
}
    
    public async updateLastCellOfCurrentDay(): Promise<void> {
        // ... (existing implementation)
        try {
            const now: Date = new Date();
            const dayOfMonth: number = now.getDate();
            const dayColumnHeader: string = `Day${dayOfMonth}`;
            console.log('dayColumnHeader-'+ 'Day${dayOfMonth}');

            // Get the header row to find the correct column
            const headerRange: string = 'A1:AE1'; // Adjust this range if your headers are not in row 1
            ...
            const headers: (string | null)[] = headerResponse.data.values?.[0] || [];
            const columnIndex: number = headers.findIndex((header: string | null) => header === dayColumnHeader);

            if (columnIndex === -1) {
                console.error(`ERROR: Column '${dayColumnHeader}' not found in the spreadsheet.`);
                return;
            }

            const columnLetter: string = String.fromCharCode('A'.charCodeAt(0) + columnIndex);
            const columnRange: string = `${columnLetter}:${columnLetter}`;
            console.log('column letter and range -'+ columnRange);

            // Get all values from the identified column
            const columnResponse = await this.sheets.spreadsheets.values.get({
                spreadsheetId: this.spreadsheetId,
                range: columnRange,
            });

            const values: (string | number | null)[][] | null = columnResponse.data.values as (string | number | null)[][] | null;
            const lastRow: number = (values && values.length > 0) ? values.length : 0;
            const timestamp: string = now.toLocaleString();
            const updateRange: string = `${columnLetter}${lastRow + 1}`;
            console.log(`Updating range: ${updateRange}`);
            ...
            console.log(`Successfully updated cell ${updateRange} with timestamp: ${timestamp}`);
        } catch (err) {
            console.error('ERROR UPDATING', err);
        }
    }
...
// Example usage:
// const manager1 = GoogleSheetManager.getInstance(SPREADSHEET_ID);
// const manager2 = GoogleSheetManager.getInstance(SPREADSHEET_ID); 
// console.log(manager1 === manager2); // This will output true
